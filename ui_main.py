...
class FEAMainWindow(QtWidgets.QMainWindow):
    ...
    def _do_mesh(self, reset_camera=True):
        ...
        n_comp = int(minfo.get("n_components", 1))
        comp_lbl = f", {n_comp} groups" if n_comp > 1 else ""
        self.lbl_mesh.setText(
            f"{minfo['n_nodes']} nodes, {minfo['n_elem']} tets{repaired}{comp_lbl}\n"
            f"quality min={q['q_min']:.2f} mean={q['q_mean']:.2f}")
        warns = list(q["warnings"])
        if n_comp > 1:
            # The mesh kept every part, but some touch only at points/edges
            # (e.g. auxetic hinges) and so are separate bonded groups.  Flag it
            # so the user knows a single load path may not span all of them.
            warns.append(
                f"Mesh has {n_comp} disconnected groups: parts that touch only "
                "at hinge points/edges are kept at full size but are not bonded "
                "across those zero-thickness joints.")
        if warns:
            self._set_status("Mesh warning: " + "; ".join(warns), 100)
        else:
            self._set_status(
