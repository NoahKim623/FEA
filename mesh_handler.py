...
class MeshHandler:
    ...
    def tetrahedralize(self, refinement: str = "medium",
        ...
        # 1. Exact path -- a clean, single, watertight, two-manifold shell can be
        #    fed straight to TetGen, preserving the geometry precisely.
        if self._is_simple_solid(surf):
            try:
                pts, tris = _polydata_to_arrays(surf)
        ...
        # 2. Robust path -- rebuild the solid as a watertight *union* of all its
        #    shells and mesh that.  This is what makes awkward STLs work: it
        #    resolves multiple/overlapping shells, self-intersections and
        #    non-manifold junctions (e.g. features modelled into a base plate)
        #    that previously collapsed to a tiny fragment or failed outright.
        if node is None:
            try:
        ...
        # 3. Last resort -- a pymeshfix triangle-soup repair (helps single-shell
        #    surfaces whose interior the voxel pass could not identify).  Skipped
        #    on very large surfaces, where pymeshfix is extremely slow and tends
        #    to collapse the geometry rather than repair it.
        if node is None and pymeshfix is not None and surf.n_faces <= 50_000:
            try:
                pts, tris = _polydata_to_arrays(surf)
                pts, tris = self._repair_arrays_pymeshfix(pts, tris)
                repaired = True
                node, elem = self._tetgen_ladder(
                    pts, tris, [dict(order=1, mindihedral=10.0, minratio=2.0),
    ...
    def _make_variants(surf, refinement, min_radius_edge, min_dihedral):
        ...
        The requested quality is tried first; if TetGen cannot satisfy it the
        easier variants still produce a usable mesh rather than failing outright.
        """
    ...
    def _remesh_union(self, surf, n_div: int = 110,
        ...
        *sign* comes from a robust ray-cast inside test (:meth:`_enclosed_mask`)
        ...
        # Thin the mesh further, but only if decimation keeps it watertight and
        # manifold (it can otherwise introduce self-intersections).
        try:
            dec = out.decimate(0.5).clean()
            if dec.n_faces and self._is_closed_manifold(dec):
                out = dec
        except Exception:                          # pragma: no cover - keep raw
            pass
        return out
